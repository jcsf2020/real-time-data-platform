...
| [.github/workflows/deploy-dbt-refresh-cloud-run.yml](../.github/workflows/deploy-dbt-refresh-cloud-run.yml) | workflow_dispatch (manual) | Builds and deploys dbt refresh job image to Cloud Run Jobs — scaffold only, not yet dispatched |
...
| [docs/dbt-refresh-cloud-run-job-plan.md](dbt-refresh-cloud-run-job-plan.md) | Terraform and deploy workflow scaffold for `rtdp-dbt-refresh-job`. Credential contract resolved: `DATABASE_URL` secret from `rtdp-database-url`; `DBT_POSTGRES_PASSWORD` secret removed. Scheduler still targets `rtdp-silver-refresh-job`. Cloud SQL still NEVER / STOPPED. |
