...
- BigQuery analytical tier, bounded backfill, incremental append, and append scheduler are
  ...
  freshness_age_hours metric NOT YET PROVEN; Cloud Monitoring alerting NOT YET PROVEN.
  Remaining BigQuery work: confirmed scheduled event execution; passing
  `freshness_max_age_hours` live run; notification delivery proof; Cloud Monitoring
  alerting policy; Dataflow is not yet implemented.
