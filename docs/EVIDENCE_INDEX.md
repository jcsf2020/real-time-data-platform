...
| [docs/dataflow-bounded-runner-proof-evidence.md](dataflow-bounded-runner-proof-evidence.md) | PACKAGING FIX v2 APPLIED (exec/dataflow-bounded-runner-proof) -- first Dataflow execution attempt (job 2026-05-23_23_00_38-6447348894342053834) failed with JOB_STATE_CANCELLED; root cause: ModuleNotFoundError: No module named 'rtdp_contracts'; BigQuery rows=0; Cloud SQL STOPPED/NEVER; schedulers PAUSED; initial fix (setup.py + --setup_file) was broken (setuptools read root pyproject.toml, overwrote name to real-time-data-platform and install_requires to []); corrected fix: --extra_packages with wheel built from packages/contracts via uv build (Name: rtdp-contracts; Requires-Dist: pydantic>=2.13.3); setup.py deleted; setuptools>=75.0 removed; 3 new packaging tests (no setup.py, wheel builds with pydantic dep, source uses extra_packages); 384 pytest passed; ruff clean; PLAN_EXIT=0; Cloud SQL STOPPED/NEVER; schedulers PAUSED; second execution pending operator approval. |
