...
| dbt metrics IAM audit | dbt-metrics-runtime-monitoring-iam-audit.md | Runtime SA confirmed; monitoring.metricWriter MISSING; IAM grant pending |
...
- [docs/dbt-metrics-runtime-monitoring-iam-audit.md](dbt-metrics-runtime-monitoring-iam-audit.md) -- AUDIT (READ-ONLY) -- dbt metrics runtime IAM audit; runtime SA `rtdp-worker-sa@project-42987e01-2123-446b-ac7.iam.gserviceaccount.com` confirmed via live GCP JSON + Terraform; `roles/monitoring.metricWriter` MISSING for `rtdp-worker-sa`; current binding held by `rtdp-terraform-plan-ci` (CI only); `DBT_METRICS_DRY_RUN=false` blocked; Terraform recommendation: `google_project_iam_member.worker_monitoring_metric_writer` in `iam.tf`; next branch `infra/dbt-metrics-runtime-monitoring-iam`; PLAN_EXIT=0; Cloud SQL STOPPED/NEVER; schedulers PAUSED; 348 tests passed; no GCP mutations
...
- dbt is the operational scheduled transformation path (accepted as of
  ...
  dbt metrics runtime IAM audit COMPLETE: runtime SA `rtdp-worker-sa@...` confirmed;
  `roles/monitoring.metricWriter` MISSING for `rtdp-worker-sa`; `DBT_METRICS_DRY_RUN=false`
  blocked until `infra/dbt-metrics-runtime-monitoring-iam` adds the binding; PLAN_EXIT=0;
  Cloud SQL STOPPED/NEVER; schedulers PAUSED; 348 tests passed; no GCP mutations
  (see `docs/dbt-metrics-runtime-monitoring-iam-audit.md`).
  Remaining dbt work: `roles/monitoring.metricWriter` IAM grant for `rtdp-worker-sa`; live metric write proof.
