...
- [docs/dbt-metrics-runtime-proof-runbook.md](dbt-metrics-runtime-proof-runbook.md) -- RUNBOOK -- dbt metrics runtime proof; PR #203 (push_dbt_metrics.py; 75 tests; dry-run safe) + PR #204 (runtime integration; DBT_METRICS_ENABLED=false default; DBT_METRICS_DRY_RUN=true default); 335 pytest passed; ruff clean; PLAN_EXIT=0; Cloud SQL STOPPED/NEVER; both schedulers PAUSED; live Cloud Monitoring writes NOT enabled; dry-run activation path documented; live write activation blocked on auth strategy + monitoring.metricWriter IAM confirmation
