...
- **dbt / transformation governance**: dbt has been validated against Cloud SQL with output parity versus stored functions (silver 256/256, gold 7/7; 22 dbt tests passed; API readback HTTP 200). Evidence: [docs/dbt-cloud-sql-validation-evidence.md](dbt-cloud-sql-validation-evidence.md). The local dbt refresh runtime package (`apps/dbt-refresh-job`, CLI `rtdp-dbt-refresh-job`) is implemented and tested locally. Terraform and deploy workflow scaffolding for `rtdp-dbt-refresh-job` (Cloud Run Job) exists as of branch `feat/dbt-refresh-cloud-run-job` — see [docs/dbt-refresh-cloud-run-job-plan.md](dbt-refresh-cloud-run-job-plan.md). Deployment, credential contract resolution (`rtdp-database-url` may be a full URL not a raw password), and scheduler switch from `rtdp-silver-refresh-job` to `rtdp-dbt-refresh-job` all remain pending for a subsequent controlled branch. Stored functions remain the authoritative refresh path until deployment evidence is accepted. Migration plan: [docs/dbt-operational-migration-plan.md](dbt-operational-migration-plan.md).
